RSpec.describe Worship, type: :system do
  describe '参拝機能' do
    ...
    describe '参拝記録の確認' do
      before do
        create(:worship, worship_day: Date.today.beginning_of_month, user_id: user_introduction.user_id,
                         shinto_id: test_shinto.id)
        create(:worship, worship_day: Date.today.months_ago(1).to_date.beginning_of_month,
                         user_id: user_introduction.user_id, shinto_id: test_shinto.id)
        create(:worship, worship_day: Date.today.months_ago(2).to_date.beginning_of_month,
                         user_id: user_introduction.user_id, shinto_id: test_shinto.id)
      end
      ...
      describe  '御朱印帳機能' do
        context '参拝した神社名と参拝日時が' do
          it '御朱印帳に記載される' do
            ...
            expect(find('.gosyuin_book',
                        visible: false)).to have_selector('.gosyuin_book_day', visible: false,
                                                                               text: worship1.worship_day.strftime('%Y年%m月%d日').tr('0123456789', '〇一二三四五六七八九'))
            expect(find('.gosyuin_book',
                        visible: false)).to have_selector('.gosyuin_book_day', visible: false,
                                                                               text: worship2.worship_day.strftime('%Y年%m月%d日').tr('0123456789', '〇一二三四五六七八九'))
            expect(find('.gosyuin_book',
                        visible: false)).to have_selector('.gosyuin_book_day', visible: false,
                                                                               text: worship3.worship_day.strftime('%Y年%m月%d日').tr('0123456789', '〇一二三四五六七八九'))
          end
        end
        context '参拝していない神社名と参拝日時は' do
          it '御朱印帳に記載されない' do
            ...
            expect(find('.gosyuin_book',
                        visible: false)).not_to have_selector('.gosyuin_book_day', visible: false,
                                                                                   text: Date.tomorrow.strftime('%Y年%m月%d日').tr('0123456789', '〇一 二三四五六七八九'))
          end
        end
      end
      describe '参拝情報の編集 削除機能' do
        let(:worship) { create(:worship, user_id: user_introduction.user_id) }
        before do
          create(:worship_param, worship_id: worship.id)
        end
        context '参拝情報をにアクセスしたとき' do
          it '初期値が表示される' do
            visit edit_worship_path(worship.id)
            expect(find('.datepicker').value).to have_content(Date.today.strftime)
            expect(page).to have_content('testMemo')
            expect(page).to have_checked_field('雨')
            expect(page.all('.form-control')[1].value).to have_content('testParamTitle')
            expect(page.all('.form-control')[2].value).to have_content('testParamMemo')
            expect(all('input', visible: false)[13].value).to eq '1'
          end
        end
        context '参拝情報を変更したとき' do
          it '変更内容が反映される' do
            visit edit_worship_path(worship.id)
            fill_in 'worship[worship_day]', with: '2021-10-10'
            choose '雪'
            fill_in 'worship[memo]', with: 'changeTestMemo'
            attach_file 'inputFile', "#{Rails.root}/spec/factories/test.jpg", make_visible: true
            fill_in 'worship[worship_params_attributes][0][title]', with: 'ChangeParamTitle'
            find('.edit_worship_stars').find("img[alt='5']").click
            fill_in 'worship[worship_params_attributes][0][memo]', with: 'ChangeParamMemo'
            click_on '参拝情報を変更する'
            sleep 0.5
            expect(page).to have_content('参拝情報を変更しました')
            expect(page).to have_content('2021-10-10')
            expect(page).to have_content('changeTestMemo')
            expect(page).to have_content('雪')
            expect(page).to have_selector("img[src$='test.jpg']")
            expect(page).to have_content('ChangeParamTitle')
            expect(page).to have_content('ChangeParamMemo')
            expect(all('input', visible: false)[0].value).to eq '4.5'
          end
        end
        context '参拝情報を削除したとき' do
          it '削除メッセージが表示され、参拝情報が削除される' do
            visit worship_path(worship.id)
            page.accept_confirm do
              find('.fa-trash').click
            end
            expect(page).to have_content('参拝記録を削除しました')
            expect(page).not_to have_content(Date.today)
          end
        end
      end
      describe '他のユーザーのアクセス' do
        before do
          logout_and_other_user_login
        end
        context '他のユーザーがユーザープロフィールにアクセスしたとき' do
          it '参拝一覧情報のリンクが現れ、クリックすると参拝一覧に飛ぶ' do
        ...
        context '他のユーザーが参拝詳細画面にアクセスしたとき' do
          it '編集ボタンと削除ボタンが表示されない' do
            worship = Worship.first
            visit worship_path(worship.id)
            expect(page).to have_no_css('.fa-trash')
            expect(page).to have_no_css('.fa-edit')
          end
        end
        context '他のユーザーが参拝編集画面にアクセスしようとしたとき' do
