class RankingItemsController < ApplicationController
  def edit
    ...
    @shinto = Shinto.find(params[:format]) if params[:format]
  ...
  def update
    ...
    redirect_to user_introduction_path(@ranking_item.ranking.user.user_introduction.user_id),
                notice: "#{@ranking_item.ranking.rank}位の神社を更新しました"
  end
  ...
  def search
    ...
    @shintos = Kaminari.paginate_array(@shintos.reject do |shinto|
                                         shintos.include?(shinto.id)
